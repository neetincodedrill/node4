...
router.get('/', async(req, res, next) => {  
    ...
    // console.log(allUser)
   const data = await  Promise.all( allUser.map(async(element) => {
        const allAddress = await addressinfo.find({email : element.email}).toArray()
        const info = {
            user : element,
            address : allAddress
        }
...
router.post("/",async(req,res) => {
    upload(req,res,async function(err){
        ...
        if(err instanceof multer.MulterError){
            ...
            return res.status(400).send('Only .png, .jpg and .jpeg format filetype  allowed!')
        }
        else{
